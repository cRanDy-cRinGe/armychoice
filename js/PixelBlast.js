/**
 * PixelBlastEffect
 * Адаптація React-компонента PixelBlast
 * для роботи з vanilla JS, Three.js та Postprocessing.
 */
class PixelBlastEffect {
    constructor(container, options = {}) {
        // Перевірка наявності бібліотек
        if (typeof THREE === 'undefined') {
            console.error("Three.js не знайдено. Переконайтеся, що він підключений.");
            return;
        }
        if (typeof POSTPROCESSING === 'undefined') {
            console.error("Postprocessing не знайдено. Переконайтеся, що він підключений.");
            return;
        }
        ...
        this.config = {
            ...
            liquidStrength: options.liquidStrength || 0.1,
            liquidRadius: options.liquidRadius || 1,
            pixelSizeJitter: options.pixelSizeJitter || 0,
            enableRipples: options.enableRipples || true,
            rippleIntensityScale: options.rippleIntensityScale || 1,
            rippleThickness: options.rippleThickness || 0.1,
            rippleSpeed: options.rippleSpeed || 0.3,
            liquidWobbleSpeed: options.liquidWobbleSpeed || 4.5,
            autoPauseOffscreen: options.autoPauseOffscreen || true,
            speed: options.speed || 0.5,
            transparent: options.transparent ?? true,
            edgeFade: options.edgeFade || 0.5,
        ...
        this.clickIx = 0;
        this.MAX_CLICKS = 10;
        this.SHAPE_MAP = { square: 0, circle: 1, triangle: 2, diamond: 3 };
    ...
    init() {
        ...
        this.uniforms = {
            ...
            uShapeType: { value: this.SHAPE_MAP[this.config.variant] ?? 0 },
            uPixelSize: { value: this.config.pixelSize * this.renderer.getPixelRatio() }, // <--- ОСЬ ТАК
        ...
        this.material = new THREE.ShaderMaterial({
            vertexShader: this.VERTEX_SRC,
            fragmentShader: this.FRAGMENT_SRC,
        ...
        // 4. Розміри та ResizeObserver
        this.setSize();
        this.resizeObserver = new ResizeObserver(this.setSize.bind(this));
        this.resizeObserver.observe(this.container);

        // 5. Ефекти (Liquid, Postprocessing)
        this.composer = null;
        this.touch = null;
        this.liquidEffect = null;

        if (this.config.liquid) {
            this.touch = this.createTouchTexture();
            ...
            this.liquidEffect = this.createLiquidEffect(this.touch.texture, {
        ...
        // (Додавання noiseAmount, якщо потрібно, тут) ...
    ...
    // Метод анімації
    animate() {
        if (this.config.autoPauseOffscreen && !this.visibilityRef.visible) {
            requestAnimationFrame(this.animate.bind(this));
            return;
        }

        const elapsedTime = this.timeOffset + this.clock.getElapsedTime() * this.config.speed;
        this.uniforms.uTime.value = elapsedTime;

        if (this.liquidEffect) {
            this.liquidEffect.uniforms.get('uTime').value = elapsedTime;
        }
    ...
    // --- Допоміжні функції (взяті з твого коду) ---
    ...
    createLiquidEffect(texture, opts) {
        const fragment = `
        ...
        // Використовуємо кастомний клас Effect, оскільки POSTPROCESSING.Effect може бути недоступний напряму
        class LiquidEffect extends POSTPROCESSING.Effect {
            constructor(texture, opts) {
                super('LiquidEffect', fragment, {
                    uniforms: new Map([
        ...
        return new LiquidEffect(texture, opts);
    }

    // --- Шейдери (взяті з твого коду) ---
